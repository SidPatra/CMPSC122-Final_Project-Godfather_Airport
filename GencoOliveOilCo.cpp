...
void main() {
	...
	if (file.is_open())
	{
		...
		while (doneCounter < 10)
		{
			...
			{
				if (file.eof() && !(WEFUCKINGBROKE))
				{
					break;
				}

				if (totalPlanes > 0 && !(WEFUCKINGBROKE))
				{
					getline(file, line);
					line += ",";
				}
			...
			if (file.eof() && (inAir.getSize() + onGround.getSize() + atRefuel.getSize()) == 0)
				doneCounter++;
			else
				doneCounter = 0;
